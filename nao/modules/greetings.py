...
from naoimport BOT_USERNAME, BOT_ID, LOGS_CHANNEL_ID, bot, dp
from nao.config import get_str_key
from nao.decorator import register
from nao.services.apscheduller import scheduler
from nao.services.mongo import db
from nao.services.redis import redis
from nao.services.telethon import tbot
from nao.stuff.fonts import ALL_FONTS
...
async def welcome_security_handler(message: Message, strings):
    if len(message.new_chat_members) > 1:
        # FIXME: naodoesnt support adding multiple users currently
        return
